...
export default function SalesDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [attendanceRes, pipelineRes, followupsRes] = await Promise.all([
      ...
      setStats({
        ...
        activeDeals: pipelineRes.data.pipeline?.filter(p => !p.done).length || 0,
        pendingFollowUps: followupsRes.data.followups?.filter(f => !f.done).length || 0,
        completedTasks: 0,
      })
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted mt-1">Here's what's happening with your sales pipeline today.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'My Attendance This Month', value: `${stats.attendanceThisMonth}%`, icon: Calendar, color: 'text-teal' },
          { label: 'Active Deals', value: stats.activeDeals, icon: FolderOpen, color: 'text-teal' },
          { label: 'Pending Follow-ups', value: stats.pendingFollowUps, icon: Clock, color: 'text-orange' },
          { label: 'Completed Tasks', value: stats.completedTasks, icon: FolderOpen, color: 'text-teal' },
        ].map((stat, index) => (
